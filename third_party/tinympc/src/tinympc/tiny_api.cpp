...
int tiny_set_soft_constraint_settings(
                    TinySolver* solver,
                    int soft_state_linear, int soft_input_linear,
                    int soft_tv_state_linear, int soft_tv_input_linear,
                    tinytype soft_state_linear_gamma, tinytype soft_input_linear_gamma,
                    tinytype soft_tv_state_linear_gamma, tinytype soft_tv_input_linear_gamma) {
    if (!solver || !solver->settings) {
        std::cout << "Error in tiny_set_soft_constraint_settings: solver/settings is nullptr" << std::endl;
        return 1;
    }

    solver->settings->soft_state_linear = soft_state_linear;
    solver->settings->soft_input_linear = soft_input_linear;
    solver->settings->soft_tv_state_linear = soft_tv_state_linear;
    solver->settings->soft_tv_input_linear = soft_tv_input_linear;

    solver->settings->soft_state_linear_gamma = std::max<tinytype>(soft_state_linear_gamma, 1e-8);
    solver->settings->soft_input_linear_gamma = std::max<tinytype>(soft_input_linear_gamma, 1e-8);
    solver->settings->soft_tv_state_linear_gamma = std::max<tinytype>(soft_tv_state_linear_gamma, 1e-8);
    solver->settings->soft_tv_input_linear_gamma = std::max<tinytype>(soft_tv_input_linear_gamma, 1e-8);
    return 0;
}
...
int tiny_set_default_settings(TinySettings* settings) {
    ...
    // Default: hard projections for linear constraints.
    settings->soft_state_linear = 0;
    settings->soft_input_linear = 0;
    settings->soft_tv_state_linear = 0;
    settings->soft_tv_input_linear = 0;
    settings->soft_state_linear_gamma = 20.0;
    settings->soft_input_linear_gamma = 20.0;
    settings->soft_tv_state_linear_gamma = 20.0;
    settings->soft_tv_input_linear_gamma = 20.0;

    // Default off to preserve current behavior unless explicitly enabled.
    settings->return_best_iterate = 0;
    settings->best_iter_lambda_pri = 1.0;
    settings->best_iter_eta_sat = 0.25;
    settings->best_iter_sat_tol = 1e-3;
    settings->admm_mode = TINY_ADMM_MODE_AUTO;
