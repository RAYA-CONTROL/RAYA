...
/**
 * Proximal soft-halfspace projection:
 * argmin_v 0.5||v-z||^2 + (gamma/2) * max(0, a^T v - b)^2
 * For gamma -> inf this approaches hard halfspace projection.
 */
tinyVector project_soft_halfspace(const tinyVector& z, const tinyVector& a, tinytype b, tinytype gamma) {
    const tinytype a_norm2 = a.squaredNorm();
    if (a_norm2 <= 1e-12) {
        return z;
    }
    const tinytype violation = a.dot(z) - b;
    if (violation <= 0.0) {
        return z;
    }
    if (gamma <= 0.0) {
        return project_hyperplane(z, a, b);
    }

    const tinytype step = (gamma * violation) / (1.0 + gamma * a_norm2);
    return z - step * a;
}

static int get_admm_mode(const TinySolver *solver) {
    if (solver == nullptr || solver->settings == nullptr) {
        return TINY_ADMM_MODE_AUTO;
    }
    const int mode = solver->settings->admm_mode;
    switch (mode) {
    case TINY_ADMM_MODE_AUTO:
    case TINY_ADMM_MODE_SIMPLE:
    case TINY_ADMM_MODE_ROBUST_LEARNED:
        return mode;
    default:
        return TINY_ADMM_MODE_AUTO;
    }
...
static bool admm_uses_tv_state_linear(const TinySolver *solver) {
    if (solver == nullptr || solver->settings == nullptr || solver->settings->en_tv_state_linear == 0) {
        return false;
    }
    const int mode = get_admm_mode(solver);
    return mode == TINY_ADMM_MODE_AUTO || mode == TINY_ADMM_MODE_ROBUST_LEARNED;
}

static bool admm_uses_tv_input_linear(const TinySolver *solver) {
    if (solver == nullptr || solver->settings == nullptr || solver->settings->en_tv_input_linear == 0) {
        return false;
    }
    const int mode = get_admm_mode(solver);
    return mode == TINY_ADMM_MODE_AUTO || mode == TINY_ADMM_MODE_ROBUST_LEARNED;
...
void update_slack(TinySolver *solver)
{
    ...
    if (solver->settings->en_state_linear) {
        const bool soft = solver->settings->soft_state_linear != 0;
        const tinytype gamma = solver->settings->soft_state_linear_gamma;
        for (int i=0; i<solver->work->N; i++) {
            for (int k=0; k<solver->work->numStateLinear; k++) {
                ...
                if (constraint_value > b) {  // Only project if constraint is violated
                    if (soft) {
                        solver->work->vlnew.col(i) = project_soft_halfspace(solver->work->vlnew.col(i), a, b, gamma);
                    } else {
    ...
    if (solver->settings->en_input_linear) {
        const bool soft = solver->settings->soft_input_linear != 0;
        const tinytype gamma = solver->settings->soft_input_linear_gamma;
        for (int i=0; i<solver->work->N-1; i++) {
            for (int k=0; k<solver->work->numInputLinear; k++) {
                ...
                if (constraint_value > b) {  // Only project if constraint is violated
                    if (soft) {
                        solver->work->zlnew.col(i) = project_soft_halfspace(solver->work->zlnew.col(i), a, b, gamma);
                    } else {
    ...
    if (admm_uses_tv_state_linear(solver)) {
        ...
        for (int i=0; i<solver->work->N; i++) {
            for (int k=0; k<solver->work->numtvStateLinear; k++) {
                ...
                if (constraint_value > b) {  // Only project if constraint is violated
                    if (soft) {
                        solver->work->vlnew_tv.col(i) = project_soft_halfspace(solver->work->vlnew_tv.col(i), a, b, gamma);
                    } else {
    ...
    if (admm_uses_tv_input_linear(solver)) {
        const bool soft = solver->settings->soft_tv_input_linear != 0;
        const tinytype gamma = solver->settings->soft_tv_input_linear_gamma;
        for (int i=0; i<solver->work->N-1; i++) {
            for (int k=0; k<solver->work->numtvInputLinear; k++) {
                ...
                if (constraint_value > b) {  // Only project if constraint is violated
                    if (soft) {
                        solver->work->zlnew_tv.col(i) = project_soft_halfspace(solver->work->zlnew_tv.col(i), a, b, gamma);
                    } else {
...
static tinytype compute_linear_violation_proxy(TinySolver *solver, const tinyMatrix &x_cand,
                                               const tinyMatrix &u_cand)
{
    tinytype sum_violation = 0.0;
    int count = 0;

    if (solver->settings->en_state_linear && solver->work->numStateLinear > 0) {
        for (int i = 0; i < solver->work->N; ++i) {
            for (int k = 0; k < solver->work->numStateLinear; ++k) {
                const tinyVector a = solver->work->Alin_x.row(k);
                const tinytype b = solver->work->blin_x(k);
                const tinytype v = a.dot(x_cand.col(i)) - b;
                sum_violation += std::max<tinytype>(0.0, v);
                count += 1;
            }
        }
    }
    ...
    if (solver->settings->en_input_linear && solver->work->numInputLinear > 0) {
        for (int i = 0; i < solver->work->N - 1; ++i) {
            for (int k = 0; k < solver->work->numInputLinear; ++k) {
                const tinyVector a = solver->work->Alin_u.row(k);
                const tinytype b = solver->work->blin_u(k);
                const tinytype v = a.dot(u_cand.col(i)) - b;
                sum_violation += std::max<tinytype>(0.0, v);
                count += 1;
            }
        }
    ...
    if (admm_uses_tv_state_linear(solver) && solver->work->numtvStateLinear > 0) {
        for (int i = 0; i < solver->work->N; ++i) {
            for (int k = 0; k < solver->work->numtvStateLinear; ++k) {
    ...
    if (admm_uses_tv_input_linear(solver) && solver->work->numtvInputLinear > 0) {
        for (int i = 0; i < solver->work->N - 1; ++i) {
            for (int k = 0; k < solver->work->numtvInputLinear; ++k) {
    ...
    return sum_violation / static_cast<tinytype>(count);
}

static tinytype compute_input_saturation_proxy(TinySolver *solver, const tinyMatrix &u_cand)
{
    if (!solver->settings->en_input_bound) {
        return 0.0;
    }
    const tinytype tol = std::max<tinytype>(solver->settings->best_iter_sat_tol, 1e-8);
    int sat = 0;
    int total = 0;
    for (int i = 0; i < solver->work->N - 1; ++i) {
        for (int j = 0; j < solver->work->nu; ++j) {
            const tinytype uij = u_cand(j, i);
            if (uij >= solver->work->u_max(j, i) - tol ||
                uij <= solver->work->u_min(j, i) + tol) {
                sat += 1;
            }
            total += 1;
        }
    ...
    return static_cast<tinytype>(sat) / static_cast<tinytype>(total);
}
...
int solve(TinySolver *solver)
{
    ...
    const bool use_best_iterate = solver->settings->return_best_iterate != 0;
    tinyMatrix best_x = solver->work->vnew;
    tinyMatrix best_u = solver->work->znew;
    tinytype best_score = std::numeric_limits<tinytype>::infinity();
    ...
    for (int i = 0; i < solver->settings->max_iter; i++)
    {
        ...
        if (use_best_iterate) {
            const tinytype pri_state = (solver->work->x - solver->work->vnew).cwiseAbs().maxCoeff();
            const tinytype pri_input = (solver->work->u - solver->work->znew).cwiseAbs().maxCoeff();
            const tinytype pri_norm = std::sqrt(pri_state * pri_state + pri_input * pri_input);
            const tinytype slack_proxy =
                compute_linear_violation_proxy(solver, solver->work->vnew, solver->work->znew);
            const tinytype sat_proxy =
                compute_input_saturation_proxy(solver, solver->work->znew);
            const tinytype score =
                slack_proxy +
                solver->settings->best_iter_lambda_pri * pri_norm +
                solver->settings->best_iter_eta_sat * sat_proxy;
            if (std::isfinite(score) && score < best_score) {
                best_score = score;
                best_x = solver->work->vnew;
                best_u = solver->work->znew;
            }
        }
        ...
        if (termination_condition(solver)) {
            ...
            solver->solution->x = use_best_iterate ? best_x : solver->work->vnew;
            solver->solution->u = use_best_iterate ? best_u : solver->work->znew;
