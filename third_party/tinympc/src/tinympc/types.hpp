...
typedef enum {
    // Legacy behavior: use en_tv_* flags directly.
    TINY_ADMM_MODE_AUTO = 0,
    // No time-varying (TV) constraints inside ADMM.
    TINY_ADMM_MODE_SIMPLE = 1,
    // Learned robust barrier is enforced as TV constraints during ADMM.
    TINY_ADMM_MODE_ROBUST_LEARNED = 3
} TinyAdmmMode;
...
typedef struct {
    ...
    // Soft-constraint options for linear and time-varying linear constraints.
    // If disabled, projections are hard (exact halfspace projection).
    // If enabled, uses proximal soft halfspace with penalty gamma.
    int soft_state_linear;
    int soft_input_linear;
    int soft_tv_state_linear;
    int soft_tv_input_linear;
    tinytype soft_state_linear_gamma;
    tinytype soft_input_linear_gamma;
    tinytype soft_tv_state_linear_gamma;
    tinytype soft_tv_input_linear_gamma;

    // Optional ADMM truncation mitigation: return best iterate seen so far
    // according to score = slack + lambda_pri*||r_pri|| + eta_sat*sat_proxy.
    int return_best_iterate;
    tinytype best_iter_lambda_pri;
    tinytype best_iter_eta_sat;
    tinytype best_iter_sat_tol;
    int admm_mode; // TinyAdmmMode
} TinySettings;
